...
export async function migrate(db: DB) {
  ...
  const clinicalFoundationSchema = await readFile(
  ...
  const heartFailureSchema = await readFile(
    new URL("./heart-failure-schema.sql", import.meta.url),
    "utf8",
  );
  const echoValveSchema = await readFile(
  ...
  await db.transaction(async (tx) => {
    ...
    const clinicalFoundationMigration = (
      await tx.query<{ checksum: string }>(
        "SELECT checksum FROM governance.migration WHERE name='004-clinical-foundation'",
      )
    ).rows[0];
    if (
      clinicalFoundationMigration &&
      clinicalFoundationMigration.checksum !== hash(clinicalFoundationSchema)
    )
      throw new Error(
        "Clinical foundation migration changed; add a new migration instead",
      );
    if (!clinicalFoundationMigration) {
      await tx.query(clinicalFoundationSchema);
      await tx.query(
    ...
    const heartFailureMigration = (
      await tx.query<{ checksum: string }>(
        "SELECT checksum FROM governance.migration WHERE name='007-heart-failure-intelligence'",
      )
    ).rows[0];
    if (
      heartFailureMigration &&
      heartFailureMigration.checksum !== hash(heartFailureSchema)
    )
      throw new Error(
        "Heart failure intelligence migration changed; add a new migration instead",
      );
    if (!heartFailureMigration) {
      await tx.query(heartFailureSchema);
      await tx.query(
        "INSERT INTO governance.migration(name,checksum) VALUES('007-heart-failure-intelligence',$1)",
        [hash(heartFailureSchema)],
      );
    }
    const echoValveMigration = (
      await tx.query<{ checksum: string }>(
        "SELECT checksum FROM governance.migration WHERE name='008-echo-valve-intelligence'",
      )
    ).rows[0];
    if (
      echoValveMigration &&
      echoValveMigration.checksum !== hash(echoValveSchema)
    )
      throw new Error(
        "Echo and valve intelligence migration changed; add a new migration instead",
      );
    if (!echoValveMigration) {
      await tx.query(echoValveSchema);
      await tx.query(
